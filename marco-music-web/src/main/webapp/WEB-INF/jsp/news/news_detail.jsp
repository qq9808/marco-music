...
<body>
	...
	<div class="search-area-wrapper">
	    <div class="search-area container">
	        <h3 class="search-header">资 讯</h3>
	        <p class="search-tag-line">新鲜资讯抢先看</p>	
	            ...
	            <div class="span8 page-content">		
                    <article class=" type-post format-standard hentry clearfix">
	                    <h1 class="post-title">
	                    	...
	                    	<a href="javascript:void(0)" class="all-title-style" style="color: #f0b70c !important;">${news.title}</a>
	                    </h1>
	                    ...
	                    <div style="text-indent: 2rem; margin-bottom: 3rem;">${news.content}</div>
                    </article>	
                    <section id="comments">	
	                    <hr><h4 id="comments-title">全部回复</h4><hr>
	                    <ol class="commentlist">		                       	
	                        <c:forEach items="${list}" var="newsReply">
		                        <li class="comment even thread-odd thread-alt depth-1 some-padding" id="li-comment-4">
		                            <c:if test="${newsReply[1] eq 2}">
			                            <article id="comment-4" class="background-color-gray">		
			                                <div class="comment-meta">
			                                    <h5 class="author">
			                                    	<cite class="fn font-weight">回复 
			                                    		<a href="javascript:void(0)" rel="external nofollow" class="url">${newsReply[8]}</a> 发表的：
			                                    	</cite>
			                                    </h5>	                                    	
			                                </div>	
			                                <div class="comment-body">
			                                	<p class="no-margin">${newsReply[9]}</p>	                                       
			                                </div>
			                            </article>
		                            </c:if>
		                            <article id="comment-4" class="article-style">	
		                                <a href="javascript:void(0)">
		                                	<c:choose>
							            		<c:when test="${empty newsReply[6]}">
							            			<img src="${baseURL}/sources/img/touxiang.png" class="avatar avatar-60 photo tx-style4" />
							            		</c:when>   
							            		<c:otherwise>
							            			<img src="${txURL}${newsReply[6] }" class="avatar avatar-60 photo tx-style5" />
							            		</c:otherwise>
							            	</c:choose> 
		                                </a>	
		                                <div class="comment-meta">
		                                    <h5 class="author">
		                                    	<cite class="fn"><a href="javascript:void(0)" rel="external nofollow" class="url">${newsReply[5]}</a></cite>
		                                    </h5>
		                                    <p class="date some-margin-top">
		                                    	<fmt:formatDate value="${newsReply[3]}" pattern="yyyy-MM-dd HH:mm" /> 
		                                    </p>	
		                                </div>	
		                                <div class="comment-body">
		                                	<p>${newsReply[2]}</p>	                                       
		                                ...
		                                <div style="text-align: right;">					                    						                    	
					                    	<a href="javascript:void(0)" onclick="reply2(${newsReply[0]})"><span class="reply">回复</span></a>
	...
	<script type="text/javascript">
		...
		function deliver() {
			...
			$.ajax({
	   			type : 'POST',
	   			url : '${baseURL}/news/reply/save',
	   			data : {
	   				newsID : "${news.id}", 
	   				content : content, 
	   				level : level,
	   				authorID : "0", 
	   				eventName : "${news.title}"
	   			},     	
	   			dataType : 'json',
