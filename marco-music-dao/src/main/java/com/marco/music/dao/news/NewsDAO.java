...
public class NewsDAO extends FatherDAO {
	...
	public List<Object[]> getTenData() {
		...
		String sql = " SELECT a.id AS newsID,a.title,a.createtime,COUNT(b.id) AS replyNum FROM article_news a LEFT JOIN article_news_reply b ON (b.news_id = a.id) GROUP BY newsID ORDER BY a.createtime DESC ";
		List<Object> values = new ArrayList<Object>();
